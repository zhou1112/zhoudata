...
react.js react核心，封装react方法

react-dom.js dom节点，实现浏览器dom操作
...
class ComponentDemo extends Component {
  render(){
    ...
    // 3. render 方法 return 回来的根元素只能是一个，超过会报错(必须放在一个闭合标签里，否则会报错)
    // 4. { } 里面可以写JS代码
    return (
      <div>
...
1.在render内`let styles={   }`

2.在class外`let styles={   }`
...
参考：react官网-->guides-->reusable components
...
// 组件调用方式(在父组件中调用子组件)
// <PropsDemo title="设置的标题" />(<子组件名 属性名=属性值)

若有多个属性，多个属性之间用空格隔开,属性值为数字用{}
<PropsDemo title="设置的标题" name="hello" num={1} />

let obj={
  hello:"hello",
  world:"world"
}
<App name={obj.hello} xxx={obj.world} />
调用：{this.props.name}{this.props.xxx}
也可使用剩余参数
<App name={{.../obj}} />
调用：{this.props.name.hello}{this.props.name.world}

在子组件中调用：{this.props.属性名}
{this.props.title}{this.props.name}{this.props.num}
```
...
![zq1](http://odowb0dhc.bkt.clouddn.com/zq1.png)
![zq2](http://odowb0dhc.bkt.clouddn.com/zq2.png)
![zq3](http://odowb0dhc.bkt.clouddn.com/zq3.png)
![zq4](http://odowb0dhc.bkt.clouddn.com/zq4.png)
...
<div ref="sss">我是div</div> //定义名字
this.refs.sss //取名字为sss的对象
